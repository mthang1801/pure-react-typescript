...
const getAction = [
	{ label: "Tạo phiếu đối soát", value: 1 },
	{ label: "Đang đối soát", value: 2 },
	{ label: "Chỉnh sửa phiếu đối soát", value: 3 },
	{ label: "Đã đối soát", value: 4 },
	{ label: "In phiếu đối soát", value: 5 }
];
...
export const columnsDataCheck2 = ({ changeRealQty, removeProduct }: any) => {
	return [
		{
			title: "STT",
			dataIndex: "id",
...
export const columnsDataHistory = () => {
	return [
		{
			title: "Thời gian",
			dataIndex: "updatedAt",
		...
		},
		{
			...
			dataIndex: "action_by",
			key: "action_by",
			render: (action_by: string, record: any, index: number) => {
				return <>{action_by}</>;
			}
		},
		{
			...
			key: "action_status",
			render: (action_status: string, record: any, index: number) => {
				return <>{getAction.find((x: any) => x.value === action_status)?.label}</>;
			}
		}
	];
};
