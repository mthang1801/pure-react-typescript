...
import { columnsListProduct } from "../edit/data";
import { columnsApplyCategory, columnsApplyProduct, columnsCoupon } from "./data";
...
const CouponCreate = () => {
	...
	const handleCreateCoupon = async (values?: any) => {
		...
		let params = {
			...
			apply_for_customer_rankings: customerSource,
	...
	const rowCouponSelection = {
		selectedRowKeys: addSelectedCouponRowKeys,
		onSelect: (_record: any, _selected: boolean, _selectedRows: any) => {
			if (_selected) {
		...
		onSelectAll: (_selected: boolean, _selectedRows: any, _changeRows: any) => {
			if (_selected) {
				...
				_changeRows.forEach((e: any) => {
					...
					_rows = _rows.filter((item) => item?.coupon_detail_code !== e?.coupon_detail_code);
				});
	...
	const checkCouponValue = (e: any) => {
		console.log(e);
		if ([...tableCode].find((x: any) => x.coupon_detail_code === e)) {
			notifyWarning("Trùng code vui lòng nhập lại");
			setAutoCouponCode("");
			formCreate.setFieldValue("coupon_code", "");
		} else {
			setAutoCouponCode(e.target.value);
		}
	};
	const contentAction = () => {
		return (
			<div style={{ borderRadius: "5px", padding: "4px", width: "140px" }}>
				...
				<div
					...
					onClick={(e: any) =>
						addSelectedCouponRowKeys.length > 0 ? setOpenDeleteCoupon(true) : notifyWarning("Vui lòng chọn mã giảm")
					}
				>
					Xóa
				</div>
			...
			>
				...
				<div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", marginTop: "8px" }}>
					...
					<div
						...
						style={{ marginLeft: "8px" }}
						onClick={() =>
							addSelectedRowKeys.length === 0 ? notifyWarning("Vui lòng chọn sản phẩm") : handleSubmitAddCategory()
						}
					>
						<SvgIconPlus style={{ transform: "scale(0.7)" }} />
						&nbsp;Thêm
						...
						<div style={{ position: "relative" }}>
							...
							{visibleSource && (
								<div style={{ position: "absolute", zIndex: "99" }}>
									<Form.Item rules={[{ required: true, message: "Vui lòng không bỏ trống" }]} name="utm_source">
										<Checkbox.Group
				...
				<div style={{ width: "calc(73% - 6.5px)" }}>
					<div className="contentSection">
						...
						<div style={{ display: "flex", justifyContent: "space-between" }}>
							...
							>
								<Input
									onBlur={(e: any) => checkCouponValue(e.target.value)}
					...
					<div className="contentSection">
						...
						<div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
							...
							>
								<div
									...
									onClick={() => setInfoSale({ ...infoSale, discount_type: 1, discount_amount: 1 })}
								>
									Theo %
								</div>
								<div
									...
									onClick={() => setInfoSale({ ...infoSale, discount_type: 2, discount_amount: 1 })}
								>
									Số tiền
								</div>
							</div>
						...
						<h4 style={{ marginTop: "10px" }}>Giới hạn sử dụng (không tick chọn nghĩa là không giới hạn)</h4>
						...
						{applyFor === 3 && (
							<div>
								...
								<TableStyledAntd
									...
									bordered
									pagination={false}
									columns={columnsApplyProduct({ removeAdd, handleChangeValue }) as any}
									widthCol1="80px"
									widthCol3="200px"
									widthCol4="60px"
								/>
					...
					<div className="contentSection">
						...
						<Form
							...
							onFinish={(values: any) =>
								setFilterTableCode({ ...filterTableCode, q: values?.q, status: values?.status, page: 1 })
							}
						...
						>
							...
							<Form.Item style={{ margin: "1px 0 13px 0", width: "calc((100% - 400px) / 3)" }}>
								<DatePicker className="defaultDate" placeholder="Ngày tạo" disabled />
							</Form.Item>
							...
							>
								<div className="searchButton" style={{ marginTop: "2px" }}>
									<Svg138 />
									&nbsp;Thao tác
								</div>
							</Popover>
							...
							>
								<div className="defaultButton" style={{ marginTop: "2px" }}>
									<SvgIconPlus style={{ transform: "scale(0.7)" }} />
									&nbsp;Thêm coupon
								</div>
							</Popover>
						...
						<TableStyledAntd
							...
							bordered
							pagination={false}
							columns={columnsCoupon({ removeCouponCode, changeStatusCouponCode }) as any}
							widthCol5="120px"
							widthCol6="60px"
							widthCol7="60px"
						/>
