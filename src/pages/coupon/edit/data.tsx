...
export const columnsCoupon = ({ defaultData, editCallback, changeStatusCouponCode, removeCouponCode }: any) => {
	return [
		{
			title: "Mã coupon",
		...
		},
		{
			...
			render: (used: string, record: any, index: number) => {
				return <div>{used && convertNumberWithDot(used)}</div>;
			}
		},
		{
			title: "Còn lại",
			...
			render: (remain: string, record: any, index: number) => {
				return <div>{remain && convertNumberWithDot(remain)}</div>;
			}
		},
		{
			title: "Trạng thái",
				...
				return (
					<div
						style={{
							...
							width: "100%",
							cursor:
								Number(defaultData.status) === 1 || Number(defaultData.status) === 2 || Number(defaultData.status) === 3
									? "pointer"
									: "not-allowed"
						}}
					>
						<Switch
							disabled={
								!(
									Number(defaultData.status) === 1 ||
									Number(defaultData.status) === 2 ||
									Number(defaultData.status) === 3
								)
							}
							checked={status}
							onChange={(e: any) => changeStatusCouponCode(e, record)}
						/>
					</div>
				);
			}
