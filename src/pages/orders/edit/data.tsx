...
export const columnsData = ({ removeProduct, setQuantityCallback, orderStatus }: any) => {
	return [
		{
			title: "STT",
			dataIndex: "seller",
		...
		},
		{
			...
			render: (desc: string, record: any, index: number) => {
				return (
					<div
						onClick={() => orderStatus === 1 && removeProduct(record)}
						style={{
							border: "1px solid rgb(212,212,212)",
							borderRadius: "2px",
							width: "24px",
							...
							alignItems: "center",
							justifyContent: "center",
							cursor: orderStatus === 1 ? "pointer" : "not-allowed"
						}}
					>
						<SvgBin fill="rgb(212,212,212)" style={{ transform: "scale(0.7)" }} />
					</div>
				);
			}
		}
...
export const columnsHistory = () => {
	return [
		...
		},
		{
			...
			key: "handled_by_name",
			render: (handled_by_name: string, record: any, index: number) => {
				return <div>{handled_by_name}</div>;
			}
		},
		{
			title: "Thao tác",
			...
			render: (order_status_name: string, record: any, index: number) => {
				return <div>{order_status_name}</div>;
			}
		}
	];
};
