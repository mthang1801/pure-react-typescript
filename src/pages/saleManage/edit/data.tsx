...
export const columnsData1 = ({
	...
	return [
		{
			title: "Đơn hàng có giá trị từ",
			dataIndex: "total_price_from",
			key: "total_price_from",
			render: (total_price_from: any, record: any, index: number) => {
				return (
					<div>
						<InputNumber
							min={0}
							max={1000000000}
							defaultValue={0}
							value={total_price_from}
							onChange={(e: any) => handleChangeValueFrom(record, e)}
							formatter={(value: any) => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
							parser={(value: any) => value!.replace(/\$\s?|(,*)/g, "")}
							className="defaultInputNumber"
						/>
		...
		{
			title: "Đơn hàng có giá trị đến",
			dataIndex: "total_price_to",
			key: "total_price_to",
			render: (total_price_to: any, record: any, index: number) => {
				return (
					<div>
						<InputNumber
							min={0}
							max={1000000000}
							defaultValue={0}
							value={total_price_to}
							onChange={(e: any) => handleChangeValueTo(record, e)}
							formatter={(value: any) => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
							parser={(value: any) => value!.replace(/\$\s?|(,*)/g, "")}
							className="defaultInputNumber"
						/>
				...
				return (
					<div>
						<InputNumber
							...
							onChange={(e: any) => handleChangeAmount(record, e)}
							formatter={(value: any) => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
							parser={(value: any) => value!.replace(/\$\s?|(,*)/g, "")}
							className="defaultInputNumber"
							addonAfter={
								<div style={{ display: "flex", width: "81px", alignItems: "center" }}>
									<div
										style={{
											...
											display: "flex",
											justifyContent: "center",
											background: record.discount_type === 1 ? colors.primary_color_1_1 : "#fff",
											alignItems: "center",
											cursor: "pointer"
										}}
										onClick={() => handleChangeType(record, 1)}
									>
										%
									</div>
									<div style={{ height: "33px", width: "1px", background: "rgb(212,212,212)" }} />
									<div
										style={{
											height: "33px",
											width: "calc(50%)",
											...
											background: record.discount_type === 2 ? colors.primary_color_1_1 : "#fff",
											alignItems: "center",
											cursor: "pointer"
										}}
										onClick={() => handleChangeType(record, 2)}
									>
										VNĐ
									</div>
								</div>
