import { Checkbox, DatePicker as DatePicker1, Form, Input, InputNumber, Modal, Select, Tree, TreeSelect } from "antd";
...
const SaleEdit = () => {
	...
	const [maxUsed, setMaxUsed] = useState<any>(undefined);
	const [selectedAddRecord, setSelectedAddRecord] = useState<any>(undefined);
	...
	useEffect(() => {
		const getCategories = async (params?: any) => {
			try {
				const response = (await api.get(`${API_URL}/promotions/${paramsUrl.id}`, params)) as any;
				let data = response?.data;
				if (data) {
					setDefaultData(data);
					setApplyAll(data?.apply_all_products);
					setMaxUsed(data?.max_use_quantity ? true : false);
					formCreate.setFieldsValue({
						program_code: data?.program_code,
						program_name: data?.program_name,
						description: data?.description,
						apply_method: data?.apply_method,
						start_date: data?.start_date ? moment(data?.start_date, "YYYY-MM-DD") : undefined,
						end_date: data?.end_date ? moment(data?.end_date, "YYYY-MM-DD") : undefined,
						start_at: data?.start_at ? moment(data?.start_at, "HH-mm-ss") : undefined,
						end_at: data?.end_at ? moment(data?.end_at, "HH-mm-ss") : undefined,
						max_use_quantity: data?.max_use_quantity,
						customer_source: data?.customer_rankings?.map((x: any) => {
							return Number(x);
						}),
						utm_sources: data?.utm_sources,
						include_dates: data?.include_dates?.map((x: any) => {
							return new Date(x).getTime();
						}),
						not_include_dates: data?.not_include_dates?.map((x: any) => {
							return new Date(x).getTime();
						}),
						days_of_week: data?.days_of_week,
						months_of_year: data?.days_of_month
					});
					setUtmSource(data?.utm_sources);
					setMondaySource(data?.days_of_week);
					setMonthSource(data?.days_of_month);
					setCustomerSource(data?.customer_rankings);
					setIncludeDays(
						data?.include_dates?.map((x: any) => {
							return new Date(x).getTime();
						})
					);
					setNotIncludeDays(
						data?.not_include_dates?.map((x: any) => {
							return new Date(x).getTime();
						})
					);
					setApplyFor(data?.apply_method);
					setEntityList(data?.entities);
					if (
						data?.include_dates?.length > 0 ||
						data?.not_include_dates?.length > 0 ||
						data?.months_of_year?.length > 0 ||
	...
	const handleCreateCoupon = async (values?: any) => {
		...
		let params = {
			...
			days_of_month: monthSource,
			months_of_year: mondaySource,
			...
			include_dates:
				includeDays?.length > 0
					? includeDays?.map((x: any) => {
							return moment(x).format("YYYY-MM-DD");
					  })
					: [],
			not_include_dates:
				notIncludeDays?.length > 0
					? notIncludeDays?.map((x: any) => {
							return moment(x).format("YYYY-MM-DD");
					  })
					: [],
		...
		try {
			const response = (await api.put(`${API_URL}/promotions/${paramsUrl.id}`, params)) as any;
	...
	const handleChangeStatus = async (e: any) => {
		try {
			...
			let params = {
				promotion_ids: [paramsUrl?.id],
				status: e
			};
			const response = (await api.put(`${API_URL}/promotions/update-multiple-promotions-status`, params)) as any;
			let data = response;
			if (data?.success) {
		...
		<div className="mainPages">
			<SubHeader
				breadcrumb={[
					...
					{ text: "Chi tiết chương trình chiết khấu" }
				]}
			/>
			...
			>
							...
							<>
								...
								<div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
									...
									<Form.Item name="start_at" style={{ margin: "0" }}>
										<DatePicker1.TimePicker className="defaultDate" onChange={(e: any) => console.log(e)} />
									</Form.Item>
				...
				<div style={{ width: "calc(73% - 6.5px)" }}>
					<div className="contentSection" style={{ margin: "0" }}>
						...
						<div style={{ display: "flex", justifyContent: "space-between" }}>
							...
							>
								<Input
									disabled
									addonAfter={<div style={{ height: "100%", cursor: "not-allowed" }}>Mã tự động</div>}
					...
					<div className="contentSection">
						<h4 style={{ margin: "0" }} onClick={() => console.log(applyAll)}>
						...
						<div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
							...
							>
								<InputNumber
									...
									onBlur={(e: any) => !e?.target?.value && setMaxUsed(false)}
									id="numberValue"
								/>
							</Form.Item>
							<Form.Item label="Số lượng khuyến mãi" style={{ width: "25%", margin: "0 0 13px 0" }} hidden={maxUsed}>
								<Input
									value="Không giới hạn"
									className="defaultInput"
									onClick={() => {
										setMaxUsed(true);
										window.document.getElementById("numberValue")?.focus();
									}}
								/>
							</Form.Item>
							<Form.Item
							...
							(applyFor === 1 ? (
								...
								>
									...
									<div
										...
										onClick={() => console.log(selectedAddRecord)}
									>
										Chiếu khấu
									</div>
									<div
