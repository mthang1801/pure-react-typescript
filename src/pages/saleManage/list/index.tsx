...
import { mailPattern, phonePattern, validateEmail } from "src/utils/helpers/functions/textUtils";
...
const SaleList = () => {
	...
	useEffect(() => {
		...
		const getCouponList = async (params?: any) => {
			try {
				...
				setListCoupon({ total: response?.paging?.total, data: data });
				setLoading(false);
			} catch (error: any) {
				setLoading(false);
	...
	const handleChangeStatus = async (e: any) => {
		try {
			let params = {
				promotion_ids: addSelectedRowKeys,
				status: e
			};
	...
	const submitSearch = (values: any) => {
		let params = values;
		params.start_date = values?.start_date ? moment(values?.start_date).format(dateFormatYMD) : undefined;
		params.end_date = values?.end_date ? moment(values?.end_date).format(dateFormatYMD) : undefined;
	...
	return (
		<div className="mainPages">
			<SubHeader breadcrumb={[{ text: "Khuyến mãi" }, { text: "Chiết khấu" }]} />
			...
			<div className="ordersPage__search" style={{ padding: "8px 16px 16px 16px" }}>
				...
				>
					...
					<Popover
						content={addSelectedRowKeys.length > 0 && addSelectedRows[0].status !== 4 && contentAction}
						title=""
						placement="bottom"
						trigger="click"
					...
					>
						<div
							...
							style={{
								marginTop: "19px",
								cursor: addSelectedRowKeys.length > 0 ? "pointer" : "not-allowed"
							}}
						>
							<Svg138 />
							&nbsp;Thao tác
					...
					<div
						onClick={() => history.push("/sale-manage/create")}
