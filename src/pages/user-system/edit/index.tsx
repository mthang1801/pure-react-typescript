...
const UserSystemEdit = ({}: any) => {
							...
							if (data?.data.functs[i]?.id == functions[j].children[k].children[l].value) {
								...
								if (a) {
									...
									arrayChecked.push({
										...
										id: functions[j].children[k].id,
										indeterminate: !(functions[j].children[k].children.length == 1) ? true : false,
	...
	return (
		<div className="mainPages">
			<SubHeader
				breadcrumb={[{ text: "Thiết lập hệ thống" }, { text: "Nhóm vai trò", link: "/roles" }, { text: "Chi tiết" }]}
			/>
				...
				{stateGetListUserGroupsPrivilege.isLoading ? (
					...
					<div className="contentSection" style={{ margin: "0" }}>
						<div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
							<p style={{ fontWeight: "bold" }} onClick={() => console.log(checkedList)}>
													...
													>
														{functionLevel1?.children &&
															functionLevel1?.children.map((functionLevel2: any) => (
																<div key={geneUniqueKey()} style={{ padding: "4px 8px " }} className="listContentChild">
																	<Checkbox
																		key={geneUniqueKey()}
																		indeterminate={
																	...
																	>
																		<div
																			...
																			onClick={() => console.log("childre", functionLevel2?.children)}
																		>
											...
											index % 2 === 1 && (
												...
												>
													...
													>
														{functionLevel1?.children &&
															functionLevel1?.children.map((functionLevel2: any) => (
																<div key={geneUniqueKey()} className="listContentChild" style={{ padding: "4px 8px " }}>
																	<Checkbox
																		key={geneUniqueKey()}
																		indeterminate={
																			checkedList.find((element) => element.id === functionLevel2?.id)?.indeterminate
																		...
																		onChange={() => {
																			onCheckAllChange(functionLevel2?.id, functionLevel1.id);
																		}}
																		checked={checkedList.find((element) => element.id === functionLevel2?.id)?.checkAll}
																		className="checkboxUserGroup"
																	>
